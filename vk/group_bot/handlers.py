    ...
    if token_:
        if is_me:
            if await edit_msg(api, update.object['message'],
                              '🔧 Обнаружен токен VkMe. Начинаю шифровку токена.'):
                metoken = token_
            else:
            ...
            if await edit_msg(api, update.object['message'],
                              '⚙️ Обнаружен токен Kate Mobile. Начинаю шифровку токена.'):
                token = token_
            else:
                return await update.reply_to_peer(
                    'Это не твой токен, либо отсутствует доступ к сообщениям'
        ...
        if is_me:
            ...
            try:
                ...
                main = False
            if main:
                msg = '👌🏻 VkMe токен сохранен и зашифрован.'
            else:
                msg = '🤔 VkMe токен сохранен, но основной токен неактуален'
        else:
            msg = '✅ Бот запущен! Приятного использования!'
...
async def switch_state(update: Update, on: bool):
    ...
    if not method.is_user(uid):
        return await update.reply_to_peer(
            '🛑 Вы не были найдены в базе данных!'
        )
